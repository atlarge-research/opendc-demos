...
uv venv --python 3.12 .venv_opendc
uv pip install --python .venv_opendc/bin/python -r requirements.txt

# Make the venv kernel discoverable by Jupyter/VS Code
.venv_opendc/bin/python -m ipykernel install --user --name opendc-demos --display-name "Python 3.12 (opendc-demos)"
